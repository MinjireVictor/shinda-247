...
import UserWrapper from "./App/Pages/Auth/UserWrapper";
import GameStat from "../src/App/Components/Game/Stat";
...
import { Routes, Route as V6Route } from "react-router-dom";
...
function App(props) {
  const { route, onWalletOpened, isLoggedIn } = props;
...
function Page(props) {
  ...
  const { route, onWalletOpened, isLogged } = props;
...
class Application extends React.Component {
  ...
  render() {
    const { t, route, onWalletOpened, isLoggedIn } = this.props;
      ...
      <BrowserRouter>
        <I18nextProvider i18n={i18next}>
          ...
          <Routes>
            ...
            >
              <V6Route index element={<Index />} />
              <V6Route
                path="register"
                element={<Register t={t} mobile={false} />}
              />
              <V6Route
                path="forgot_password"
                element={<Forgot t={t} mobile={false} />}
              />
              <V6Route
                path="wallet"
                element={<Wallet t={t} mobile={false} route={route} />}
              />
              <V6Route
                path="game"
                element={<GameStat t={t} mobile={false} />}
              />
            </V6Route>
          </Routes>
          <ReactNotification />
          ...
          <Modal.Body>
            ...
            {loading ? (
              <>
                <div className="text-center" style={{ minHeight: 200 }}>
                  <div
                    className="spinner-border text-info my-2 user-loader"
                    role="status"
                  />
                </div>
              </>
...
class PlayerRow extends React.Component {
  ...
  render() {
    ...
    if (profit === 0 || profit === 0.0 || profit === "0.00") {
      profit = "-" + array.amount;
      color = "text-warning";
    }
...
const mapDispatchToProps = (dispatch) => ({
  onWalletOpened: (route) => dispatch(setRoute(route)),
});
