...
class Deposit extends React.Component {
  ...
  stkResponse = (data) => {
    if (this._isMounted) {
      if (data.status.responseCode === 0) {
        this.setState({
          status: data.status.responseDescription,
          submitted: false,
          cls: "alert-success",
        });
        wait(25000).then(() => {
  ...
  submitForm(e) {
    ...
    if (amount < 1) {
      this.setState({
        status: "minimum top up amount is 1.00 KSH",
        disabled: false,
        cls: "alert-warning",
      });
      return;
    ...
    this.setState({
      status: "please wait, initiating STK push to " + phone + "..",
      cls: "bg-secondary",
    });
    socket.emit(
      C.STK_DEPOSIT,
  ...
  render() {
    const { userid, submitted, phone, amount, disabled, status, cls } =
      this.state;
    return (
      <>
            ...
            <>
                  ...
                  >
                    ...
                    <div className="text-center">
                      <button
                        ...
                        disabled={disabled}
                      >
                        <i className="fas fa-arrow-down mr-1 float-left font-18" />{" "}
                        Make Deposit
                      </button>
                    </div>
              ...
              <div className="alert bg-cs">
                ...
                <p className="mb-0 font-12">
                  1. Go to Mpesa menu and Select Lipa na M-PESA, then select Pay
                  Bill option
                </p>
                <p className="mb-0 font-12">
                  2. Enter business number as{" "}
                  <span className="text-yellow">{PAYBILL}</span>
                </p>
                <p className="mb-0 font-12">
                  3. Enter the account number as{" "}
                  <span className="text-yellow">Your Phone Number</span>
                </p>
                <p className="mb-0 font-12">
