...
class ManualBet extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      added: false,
      holding: false,
      payout: forceSatoshiFormat(
        storage.getKey("odd") ? storage.getKey("odd") : 1.01
      ),
      amount: forceSatoshiFormat(
        storage.getKey("lam") ? storage.getKey("lam") : 10.0
      ),
  ...
  handleClickOutside(event) {
    if (this.wrapperRef && !this.wrapperRef.current.contains(event.target)) {
      ...
      if (amount === "") {
        this.setState({ amount: 10.0 });
      }

      if (parseFloat(amount) < 10.0) {
        this.setState({ amount: forceSatoshiFormat(10.0) });
      }

      if (parseFloat(amount) <= 0) {
        this.setState({ amount: forceSatoshiFormat(10.0) });
      } else {
        if (amount === "NaN") amount = 10.0;
      ...
      if (im_in_game === true || this.state.clicked) {
        ...
        this.state.buttonProgress = setInterval(
          function () {
            let calc = amount * (Game["current_amount"] - 1);
            self.setState({
              buttonText: "CashOut " + forceSatoshiFormat(calc) + " KES",
            });
            counter++;
            ...
            //Helper
            if (
              parseFloat(Game["current_amount"]) >=
              parseFloat(this.state.payout)
            )
              this.cashOut();
          }.bind(this),
          50
  ...
  handleBet(e) {
    if (this._isMounted) {
      ...
      let { payout, holding, gameStatus, token } = this.state;
      let { im_in_game } = this.props;

      // Check User
      if (!token) {
        return sendNotfication(
          "please login in first to be able to place a bet",
          "warning",
          "top-right"
        );
  ...
  handleHotKey(e) {
    if (this._isMounted) {
      ...
      if (storage.getKey("hotkey") === "ON") {
        ...
        if (char === 102) {
          const calc = parseFloat(this.state.payout * 2);
          const max = Math.max(1.0, calc);
        ...
        >
            ...
            <Row xl={6} md={7} sm={12}>
              ...
              >
                <div className="input-group">
                  ...
                  />
                  <div className="input-group-append">
                    <span className="input-group-text w-100">X</span>
                  </div>
                </div>
              </div>
            </Row>
          ...
          <Col>
            <Row md={7} sm={12} className="m-auto text-center">
              <div
                className={"form-group rev ovh bdr-lg mt-0 mb-0"}
                style={{ width: "100%" }}
              >
                <Button
              ...
              <Row md={12} sm={12} className="m-auto">
                <div className="form-inline">
                  <Col md={12} sm={12} className="font-12 text-grey">
                    <span
                      className={"badge badge-info rounded cp"}
