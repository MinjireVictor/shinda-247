...
import { getElement, __, wait, defaultAvatar, Event, sendNotfication } from "../../../../Helper";
...
class Messages extends React.Component {
    ...
    setLoader = () => {
        if(this._isMounted)
        {
			const icon = <div className="flex-center w-100 h-100"><div className="spinner-border text-info" role="status" /></div>;
    ...
    getChats(data) {
        if(this._isMounted){
            ...
            sortByTime(data).forEach((message, i) => {
                this.setState(state => ({ messages: [<Message t={this.props.t} key={i} idx={i} message={message} redux={this.props} />, ...state.messages] }));
            });
        }
    }
        ...
        if(this._isMounted){
            ...
            if(__.upperCase(message.country) === __.upperCase(currentCountry))
            {
                let add = this.state.messages.concat([
                    <Message t={this.props.t} idx={this.state.messages.length + 1} message={message} redux={this.props} />
                ]);
    ...
    render(){
        return(
                <Element name="chatBarElement" className="h-100">
                    <ul className="nav chats h-100 flex-column" id={'sc'}>
           ...
           message = <div className="share-result">
                { isLost ?
                    ...
                    <h4 className="text-warning text-center mt-0 mb-3 trmg">
                        <img src="/assets/images/contest.png" alt="" className="img-fluid" style={{ width: 40 }} /> <br/> {t('winner')}
                    </h4>
                }
                <Row>
                    <Col md="6" sm="6">
                    ...
                    <Col md="6" sm="6">
                        <div className="text-white bg-soft-info rounded py-2">
                            <img className="mini-coin-8 mr-1" alt="" src={'/assets/images/' + __.lowerCase(message.coin) + '.png'} />
